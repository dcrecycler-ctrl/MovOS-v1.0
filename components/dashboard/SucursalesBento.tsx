...
import { LocationStat, PartsRequest } from '@/lib/supabase/queries/dashboard'
...
const util = (loc: LocationStat) =>
  loc.units > 0 ? Math.round((loc.assigned / loc.units) * 100) : 0

const toneFor = (u: number): Tone => u >= 75 ? 'green' : u >= 65 ? 'blue' : 'amber'
...
function BranchScrollCard({ loc, onClick }: { loc: LocationStat; onClick: () => void }) {
  const u = util(loc)
  const tone = toneFor(u)
  return (
    <SoftCard padding={14} onClick={onClick} style={{ flexShrink: 0, width: 152 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        <span style={{ fontFamily: 'var(--font-inter)', fontSize: 12, color: B.ink, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1, marginRight: 8 }}>
          {titleCase(loc.name).split(' ').slice(0, 2).join(' ')}
        </span>
        <Ring value={u} color={TONE_COLOR[tone]} size={32} stroke={3} label={`${u}`} />
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontFamily: 'var(--font-inter)', fontSize: 10, color: B.ink3 }}>
        <span style={{ display: 'flex', alignItems: 'center', gap: 3 }}><Dot tone="green" size={4} /> {loc.available}</span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 3 }}><Dot tone="blue"  size={4} /> {loc.assigned}</span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 3 }}><Dot tone="rose"  size={4} /> {loc.maintenance}</span>
      </div>
    </SoftCard>
  )
}

function BranchGridCard({ loc, index, onClick }: { loc: LocationStat; index: number; onClick: () => void }) {
  const u = util(loc)
  const tone = toneFor(u)
          ...
          <div>
            {isMain && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                ...
                <span style={{ fontFamily: 'var(--font-inter)', fontSize: 11, color: B.ink4 }}>{loc.city}</span>
              </div>
            )}
            <h3 style={{
            ...
            }}>{titleCase(loc.name)}</h3>
            {!isMain && <div style={{ fontFamily: 'var(--font-inter)', fontSize: 11, color: B.ink4, marginTop: 3 }}>{loc.city}</div>}
            {isMain && <p style={{ fontFamily: 'var(--font-inter)', fontSize: 13, color: B.ink3, margin: '6px 0 0' }}>{loc.city}, Uruguay</p>}
          </div>
          <Ring value={u} color={TONE_COLOR[tone]} size={isMain ? 76 : 56} stroke={isMain ? 6 : 5} label={isMain ? `${u}%` : `${u}`} />
        </div>
        {isMain ? (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 0 }}>
            {([
              { l: 'Total', v: loc.units, tone: 'sky' as Tone },
              { l: 'Disponibles', v: loc.available, tone: 'green' as Tone },
              { l: 'Asignados', v: loc.assigned, tone: 'blue' as Tone },
              { l: 'Inactivos', v: loc.maintenance, tone: 'rose' as Tone },
            ]).map((s, i) => (
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
            {([
              { l: 'Tot', v: loc.units, tone: 'sky' as Tone },
              { l: 'Disp.', v: loc.available, tone: 'green' as Tone },
              { l: 'Asig.', v: loc.assigned, tone: 'blue' as Tone },
              { l: 'Inac.', v: loc.maintenance, tone: 'rose' as Tone },
            ]).map(s => (
              <div key={s.l}>
...
export function SucursalesBento({ locations, parts, openModal }: Props) {
  return (
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 8, marginBottom: 14 }}>
        <div>
          ...
          <p style={{ fontFamily: 'var(--font-inter)', fontSize: 13, color: B.ink3, marginTop: 4, marginBottom: 0 }}>
            Utilización actual · {locations.length} {locations.length === 1 ? 'ubicación' : 'ubicaciones'} en Uruguay
          </p>
        </div>
      ...
      {locations.length === 0 ? (
        ...
        <>
          <div className="flex sm:hidden gap-2.5 overflow-x-auto pb-2 mb-3.5" style={{ marginLeft: -16, marginRight: -16, paddingLeft: 16, paddingRight: 16, scrollbarWidth: 'none' }}>
            {locations.map(loc => (
              <BranchScrollCard key={loc.id} loc={loc} onClick={() => openModal({ kind: 'branch', branch: loc })} />
            ))}
          </div>

          <div className="hidden sm:grid sm:grid-cols-2 lg:grid-cols-4 gap-3.5 mb-3.5 items-stretch">
            {locations.map((loc, i) => (
              <BranchGridCard key={loc.id} loc={loc} index={i} onClick={() => openModal({ kind: 'branch', branch: loc })} />
            ))}
          </div>
        </>
      )}
      ...
      <div className="min-w-0 w-full overflow-hidden">
        <SoftCard padding={26}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
            ...
            <PillButton tone="ghost" size="sm" onClick={() => openModal({ kind: 'all-parts', parts })}>Ver almacén</PillButton>
          </div>

          {parts.length === 0 ? (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px 0', fontFamily: 'var(--font-inter)', fontSize: 14, color: B.ink3 }}>
              Sin pedidos activos
            </div>
          ) : (
